...
export class PlanificationService {
    ...
    getAll() {
        return this.http.get(`${this.env.url}listPlanifications`, this.httpOptions).pipe(retry(0), catchError(this.handleError));
    }


addPlanification(data){   
    return this.http.post(`${this.env.url}addPlanification`, data, this.httpOptions2).pipe(retry(0), catchError(this.handleError)).toPromise();
}

deletePlanification(id){    
    return this.http.get(`${this.env.url}planifications/delete/${id}`, this.httpOptions2).pipe(retry(0), catchError(this.handleError)).toPromise();
}
