...
export class CampagneService {
    ...
    listCampagnesAvecScenarios() {
        return this.http.get(`${this.env.url}listCampagnesAvecScenarios`, this.httpOptions).pipe(retry(0), catchError(this.handleError));
    }


    addCampagne(data){
    return this.http.post(`${this.env.url}addCampagne`, data, this.httpOptions2).pipe(retry(0), catchError(this.handleError)).toPromise();
    }

deleteCampagnes(id){
    return this.http.get(`${this.env.url}campagnes/delete/${id}`, this.httpOptions2).pipe(retry(0), catchError(this.handleError)).toPromise();
}

deleteScenario(data){
    return this.http.post(`${this.env.url}campagnes/removeScenario`,data, this.httpOptions2).pipe(retry(0), catchError(this.handleError)).toPromise();
}
