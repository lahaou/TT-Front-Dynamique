...
getAll() {
    return this.http.get(`${this.env.url}listParametres`, this.httpOptions).pipe(retry(0), catchError(this.handleError));
}


paramsUpdate(data){
    return this.http.post(`${this.env.url}paramsUpdate`, data, this.httpOptions2).pipe(retry(0), catchError(this.handleError)).toPromise();
}
